...
interface NFTDetailListNFTServiceProps {
  NFTsItem: {
...
export default function NFTDetailListNFT() {
  ...
  const { NFTsItem } = useNFTsItem({
    key: `nft_detail_list/${nft_id}/${collection_id}`,
...
function NFTDetailListNFTService({
  NFTsItem,
  MetaCollection,
}: NFTDetailListNFTServiceProps) {
  const { nft_id, collection_id } = useParams();
          ...
          >
            ...
            >
              ...
              <Center justifyContent="space-between">
                <Text color="shader.a.900">{currentMetaNFT?.title || '-'}</Text>
